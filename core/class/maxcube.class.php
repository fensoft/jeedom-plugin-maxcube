...
class maxcube extends eqLogic {
    ...
    public static function startDaemon() {
      ...
      $cmd = "cd " . $path . "/3rdparty/maxcube && bash daemon.sh start " . config::byKey('maxcube_ip', 'maxcube') . " " . config::byKey('maxcube_port', 'maxcube') . " " . config::byKey('socketport', 'maxcube') . " - \"" . $url . "\"";
    ...
    public function toHtml($_version = 'dashboard') {
      return eqLogic::toHtml($_version)/* . "<script>$('[data-eqlogic_id=37]').find('span.cmdName').hide()</script>"*/;
    }
      ...
      if (init("method") == "update" && init("property") != "lastUpdate") {
        ...
        switch (init("property")) {
          case "setpoint":
            ...
            //$setpointlogic = maxcubeCmd::byEqLogicIdAndLogicalId($elogic->getId(), "setpoint");
            //$setpointlogic->execute(array('slider'=>init("value")));
            break;
          case "temp":
            $templogic = maxcubeCmd::byEqLogicIdAndLogicalId($elogic->getId(), "temp");
            $templogic->event(init("value"));
            break;
          case "valve":
